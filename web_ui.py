...
def detail_idea(progress=gr.Progress()):
    ...
    try:
        ...
        progress(0.3, desc="加载最优想法和相关论文的分析...")
        progress(0.5, desc="使用DeepSeek详细化想法中（可能需要2-3分钟）...")
...
def create_ui():
    ...
    with gr.Blocks(title="AgentColab - 自动论文处理系统") as app:
        ...
        # ==================== Tab 5: 想法详细化 ====================
        with gr.Tab("📝 想法详细化"):
            gr.Markdown("## 详细化最优想法（DeepSeek）")
            gr.Markdown("""
            将最优想法结合相关论文的分析内容进行详细化
            
            **流程**:
            1. 读取最优想法和使用的论文列表
            2. 加载相关论文的**分析内容**（而不是原文）
            3. 按格式组织：【Paper_1】论文名：分析... 【Paper_2】论文名：分析... 【创新想法】...
            4. 使用DeepSeek详细化想法
            
            **Prompt**: "我先给你n篇文章，然后再给你根据这n篇文章结合产生的idea，最后你把这个idea详细化，涉及公式和理论要进行推导。"

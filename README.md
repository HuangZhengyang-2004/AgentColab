...
#### 功能概述
将筛选出的最优想法详细化为完整的研究方案，包含理论推导、实验设计和预期贡献。
...
1. 调用 `IdeaSelectorAgent` 获取最优想法和源论文
2. 从 `data/collections/all_papers_analyzed.json` 加载源论文的分析内容
3. 使用**多轮对话**方式调用LLM：
   - 第1步：发送任务说明
   - 第2-n步：逐篇发送论文分析
   - 第n+1步：发送想法和详细化要求
4. 保存详细化结果到 `data/ideas/detailed_idea.md` 和 `.json`

#### 生成的文件

```
data/ideas/
├── detailed_idea.md             # Markdown格式的详细化内容
└── detailed_idea.json           # JSON格式（含元数据）
```
...
pipeline:
  idea_detailing:
    api_provider: "deepseek"     # deepseek, gemini, claude, gptsapi
...
detailed_idea = agent.run()  # 自动读取最优想法并详细化

# 查看结果
print(detailed_idea[:500])
```
...
#### 功能概述
使用 **Aider-chat** 自动将详细化的想法转换为可运行的Python代码，并自动执行和评价。

#### 核心特性

**1. 自动代码生成**
- 使用Aider AI编程助手
- 支持多种LLM后端（DeepSeek、Gemini、Claude）
- 生成完整的Python脚本（含import、注释、错误处理）
...
```
data/code/
├── generated_code_20241202_123456.py    # 生成的Python代码
├── generation_result_20241202_123456.json  # 生成结果元数据
├── metrics.json                         # 评价指标
├── figure_1.png                         # 图表1
├── figure_2.png                         # 图表2
└── ...
...
```
logs/
└── code_error_20241202_123456.log       # 错误日志
...
2. ✅ 包含所有必要的import语句
3. ✅ 实现文章中描述的核心算法
4. ✅ 生成评价指标（准确率、F1、MSE等）
5. ✅ 使用matplotlib绘制评价指标图表
6. ✅ 将指标保存到 `metrics.json`
7. ✅ 将图表保存为 `figure_*.png`
8. ✅ 详细的代码注释
9. ✅ try-except错误处理
10. ✅ 在最后打印"实验完成！"
...
pipeline:
  code_generation:
    api_provider: "deepseek"     # deepseek, gemini, claude
    model: "deepseek-chat"       # 模型名称
    temperature: 0.7             # 温度参数
...
result = agent.run()  # 自动读取detailed_idea.md并生成代码

if result['success']:
    print(f"✅ 代码生成成功: {result['code_file']}")
    print(f"📊 指标文件: {result['metrics_file']}")
    print(f"📈 图表数量: {len(result['figures'])}")
else:
    print(f"❌ 失败: {result['error']}")
    print(f"🐛 GitHub Issue: {result.get('github_issue')}")
```

#### 注意事项

- ⚠️ **Token消耗**：Aider会根据代码复杂度消耗大量token
- ⚠️ **执行超时**：代码执行限制5分钟
- ⚠️ **依赖安装**：确保已安装 `aider-chat`、`matplotlib`、`pandas` 等
- ⚠️ **Git配置**：如需自动推送到GitHub，需配置Git远程仓库

---

### 8. 监督评价模块 (`SupervisorAgent`)

**状态**：✅ 已完全实现

#### 功能概述
使用LLM自动评价代码生成的实验指标，判断结果的合理性和可信度，并提供改进建议。
...
1. 读取 `data/code/metrics.json`（最新的指标文件）
2. 使用LLM评价指标的合理性
3. 生成Markdown格式的评价报告
4. 保存到 `data/code/evaluation_*.md` 和 `.json`

#### 生成的文件

```
data/code/
├── evaluation_20241202_123456.md       # Markdown评价报告
└── evaluation_20241202_123456.json     # JSON格式（含元数据）
```
...
**时间**: 2024-12-02 12:34:56
**指标文件**: data/code/metrics.json
**代码文件**: data/code/generated_code_20241202_123456.py
...
# 指定特定的指标文件
result = agent.run(
    metrics_file="data/code/metrics.json",
    code_file="data/code/generated_code_20241202_123456.py"
)
...
1. ✅ `CodeGeneratorAgent` 成功运行代码后
2. ✅ Web UI中点击"监督评价"按钮
3. ✅ 手动调用 `SupervisorAgent.run()`
